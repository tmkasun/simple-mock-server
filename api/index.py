...
try:
    ...
    print(e)
    print(
        "WARN: Can't find python_digest module , Hence you won't able to use Digest Auth feature"
    )
...
try:
    ...
    print(
        "WARN: Can't find jwt module , Hence you won't able to use JWT decode feature"
    )
...
class handler(server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        requestPath = self.path.split("/")[1].lower()
    ...
    def formatResponsePayload(self):
        ...
        if accept_header is not None and accept_header.lower().split(";")[0] in [
            "application/json",
            "application/xml",
        ]:
        ...
        if self.path.split("/")[-1].find(".") != -1:
            ...
            if not len(files) == 0:
                if (
                    extention == "gz"
                ):  # if request for gzip file for testing gzip compression and https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Encoding
    ...
    def getBody(self):
        ...
        self.body = (
            None
            if content_length in [0, -1]
            else self.rfile.read(content_length).decode("UTF-8")
        )
        return self.body
    ...
    def decodeAuthHeader(self):
        ...
        if auth_type.lower() == "basic":
            auth_params = (
                base64.b64decode(auth_header.split(" ")[-1]).decode("UTF-8").split(":")
            )
